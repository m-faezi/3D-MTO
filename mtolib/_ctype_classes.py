...
def init_classes(d_type):
    ...
    MtPixel._fields_ = [
        ("location", MtImageLocation),
        ("value", pixel_type)
    ]

    MtHeap._fields_ = [
        ("entries", ct.POINTER(MtPixel)),
    ...
    MtConnectivity._fields_ = [
        ("neighbours", ct.POINTER(ct.c_int)),
        ("height", ct.c_int),
        ("width", ct.c_int),
        ("depth", ct.c_int)
    ]

    MtNodeAttributes._fields_ = [
        ("volume", float_type),
        ("power", float_type)
    ]
    ...
    MtObjectData._fields_ = [
        ("mt", ct.POINTER(MtData)),
