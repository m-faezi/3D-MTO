...
#define MT_INDEX_OF(PIXEL) (((PIXEL).location.z * mt->img.width * mt->img.height) + \
...
const int mt_conn_4[MT_CONN_4_DEPTH][MT_CONN_4_HEIGHT][MT_CONN_4_WIDTH] =
    {
        {
            0, 0, 0,
            0, 1, 0,
    ...
    for (z = 0; z != mt->img.depth; ++z)
    {
        for (y = 0; y != mt->img.height; ++y)
        {
            for (x = 0; x != mt->img.width; ++x)
            {
                ...
                // Convert from x, y, z coordinates to an array index
                INT_TYPE index = (z * mt->img.width * mt->img.height) + (y * mt->img.width) + x;
...
static int mt_queue_neighbour(mt_data* mt, PIXEL_TYPE val, SHORT_TYPE x, SHORT_TYPE y, SHORT_TYPE z)
{
...
static void mt_queue_neighbours(mt_data* mt, mt_pixel* pixel)
{
    ...
    INT_TYPE radius_z = mt->connectivity.depth / 2;
    //INT_TYPE radius_z = 1;
    ...
    // Ditto for z
    INT_TYPE conn_z_min = 0;
    if (pixel->location.z < radius_z)
        conn_z_min = radius_z - pixel->location.z;
    ...
    INT_TYPE conn_z_max = 2 * radius_z;
    if (pixel->location.z + radius_z >= mt->img.depth)
          conn_z_max = radius_z + mt->img.depth - pixel->location.z - 1;

    INT_TYPE conn_z;
    // Conn coordinates refer to position with connectivity grid
    for (conn_z = conn_z_min; conn_z <= conn_z_max; ++conn_z)
    {
        INT_TYPE conn_y;
        // Conn coordinates refer to position with connectivity grid
        for (conn_y = conn_y_min; conn_y <= conn_y_max; ++conn_y)
        {
            ...
            for (conn_x = conn_x_min; conn_x <= conn_x_max; ++conn_x)
            {
                ...
                if (mt->connectivity.
                    neighbors[(conn_z * mt->connectivity.width * mt->connectivity.height) + (conn_y * mt->connectivity.width) + conn_x] == 0)
                {
                    continue;
                }
                ...
                if (mt_queue_neighbour(mt, pixel->value,
                    ...
                    pixel->location.z - radius_z + conn_z))
                {
                    return;
                }
            }
...
void segfault_handler(int signal) {
    printf("Segmentation fault occurred. Continuing execution.\n");
    exit(1); // Or handle the error gracefully and continue execution
}

static void mt_merge_nodes(mt_data* mt, INT_TYPE merge_to_idx, INT_TYPE merge_from_idx)
{
    // Merge two nodes
    mt_node *merge_to = mt->nodes + merge_to_idx;
    mt_node_attributes *merge_to_attr = mt->nodes_attributes + merge_to_idx;
    mt_node *merge_from = mt->nodes + merge_from_idx;
    //signal(SIGSEGV, segfault_handler);
    mt_node_attributes *merge_from_attr = mt->nodes_attributes + merge_from_idx;
    merge_to->area += merge_from->area;
    FLOAT_TYPE delta = mt->img.data[merge_from_idx] - mt->img.data[merge_to_idx];
    merge_from_attr->power += delta * (2 * merge_from_attr->volume + delta * merge_from->area);
...
void mt_init(mt_data* mt, const image* img)
{
    ...
    mt->connectivity.depth = MT_CONN_4_DEPTH;
